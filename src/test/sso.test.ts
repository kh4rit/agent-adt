...
class Provider implements CookieProvider {
  ...
  refreshes: string[] = []
  constructor(
    private current = "MYSAPSSO2=t1; SAP_SESSIONID_FD2_100=s1",
    private next = "MYSAPSSO2=t2; SAP_SESSIONID_FD2_100=s2"
  ) {}
  async cookieHeader() {
    return this.current
  }
...
test("cookie client strips credentials, sends provider cookies and keeps cookies SAP issues", async () => {
  const delegate = new Delegate([ok("<a/>", { "set-cookie": ["SAP_SESSIONID_FD2_100=fresh; path=/", "sap-usercontext=sap-client=100; path=/"] }), ok("<b/>")])
  ...
  assert.equal(sent.headers?.Cookie, "MYSAPSSO2=t1; SAP_SESSIONID_FD2_100=s1")
  await client.request({ url: "/sap/bc/adt/x" } as HttpOptions)
  const jar = parseCookieHeader(delegate.received[1].headers?.Cookie)
  assert.equal(jar.get("SAP_SESSIONID_FD2_100"), "fresh")
...
test("SAML redirect page triggers exactly one re-logon and retry with the new cookies", async () => {
  ...
  assert.equal(delegate.received[1].headers?.Cookie, "MYSAPSSO2=t2; SAP_SESSIONID_FD2_100=s2")
...
test("end to end: ADT client logs on through the cookie client and recovers from an expired session", async () => {
  ...
  fake.intercept = call => (/SAP_SESSIONID_FD2_100=s1/.test(call.headers.Cookie ?? "") ? ok(SAML_PAGE, html) : undefined)
  const provider = new Provider()
...
test("cookie cache helpers: host filter, staleness, persistence", () => {
  ...
  const cookies = [
    { name: "SAP_SESSIONID_FD2_100", value: "s", domain: "sap.example.com", path: "/", expires: -1 },
  ...
  assert.deepEqual(
    sap.map(c => c.name),
    ["SAP_SESSIONID_FD2_100", "MYSAPSSO2"]
  )
  assert.equal(hasSapSession(sap), true)
  assert.equal(hasSapSession(cookiesForHost(cookies, "other.host")), false)
  assert.equal(toCookieHeader(sap), "SAP_SESSIONID_FD2_100=s; MYSAPSSO2=t")
  const cache: CookieCache = { system: "FD2", host: "sap.example.com", capturedAt: now, cookies }
  ...
  const file = join(dir, "nested", "fd2.cookies.json")
...
test("auth modes are validated in config file and environment", () => {
  const sso = parseConfigFile(JSON.stringify({ systems: { FD2: { url: "https://sap:443", username: "u", auth: "sso", sso: { headless: false, timeout: "120" } } } }), {})
